#!C:\Program Files (x86)\FontForgeBuilds\bin\fontforge.exe -script

import sys
import os.path
import json

IMPORT_OPTIONS = ('correctdir')
...
def addGlyphs(font, config):
    for k, v in config['glyphs'].items():
        ...
        g.importOutlines(src,simplify=False,scale=False)
        # Copy attributes
...
def main(config_file):
    ...
    for outfile in config['output']:
        ...
        font.generate(outfile,flags=('no-hints',))
